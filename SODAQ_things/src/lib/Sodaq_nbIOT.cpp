...
bool Sodaq_nbIOT::getIMEI(char* buffer, size_t size)
{
    ...
    println("AT+CGSN=1");
    return (readResponse<char, size_t>(_csgnParser, buffer, &size) == ResponseOK);
}
...
ResponseTypes Sodaq_nbIOT::_csgnParser(ResponseTypes& response, const char* buffer, size_t size,
    char* stringBuffer, size_t* stringBufferSize)
{
    if (!stringBuffer || !stringBufferSize) {
        return ResponseError;
    }
    ...
    char result[30 + 1];
    if (sscanf(buffer, "+CGSN: %" STR(sizeof(result) - 1) "s", result) == 1) {
        strcpy(stringBuffer, result);
        return ResponseEmpty;
    }
...
void Sodaq_nbIOT::setUrat(uint8_t urat)
{
    _urat = urat;
}
